...
use dsyrs::{
    ControlMode, Direction, DsyrsSyncClient, JogConfig, MultiSegOperationMode,
    MultiSegPositionMode, SegmentConfig, ServoConfig, ServoState, Slave,
};
use std::thread;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Jog speed: {} rpm, Accel/Decel: {} ms",
        jog_config.speed, jog_config.accel_time
    );
    ...
    println!(
        "Segment 1: {} pulses at {} rpm",
        segment1.displacement, segment1.speed
    );
    ...
    println!(
        "Segment 2: {} pulses at {} rpm",
        segment2.displacement, segment2.speed
    );
    ...
    println!(
        "Segment 3: {} pulses at {} rpm",
        segment3.displacement, segment3.speed
    );
...
fn display_status(servo: &mut DsyrsSyncClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Electrical Angle: {:.1}°",
        status.electrical_angle as f32 * 0.1
    );
    Ok(())
}
